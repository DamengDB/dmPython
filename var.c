...
fun_end:
    ...
    if (size2 > INT_MAX)
    {
        ...
        aq_sprintf(buffer, 200, "dmVar_TypeByValue(): invalid date len %lld", size2);
        PyErr_SetString(g_NotSupportedErrorException, buffer);
    }
    else
    {
        aq_sprintf(buffer, 200, "dmVar_TypeByValue(): unhandled data type %.*s", 150,
...
)
{
    ...
    aq_sprintf(buffer, 100, "dmVar_TypeBySQLType: unhandled data type %d",
            sqlType);
    PyErr_SetString(g_NotSupportedErrorException, buffer);
    return NULL;
}
